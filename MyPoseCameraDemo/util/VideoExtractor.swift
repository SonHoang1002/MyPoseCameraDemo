...
final class VideoExtractor {
    ...
    /// Đây là bản đã TỐI ƯU chất lượng (full resolution, đúng transform, đúng thời điểm).
    ...
    /// Trích 1 frame với generator mặc định của AVFoundation (KHÔNG cấu hình gì thêm).
    /// Dùng để làm đối chứng so sánh với bản đã tối ưu ở trên:
    /// - Không set tolerance -> có thể lấy frame gần đó thay vì đúng thời điểm
    /// - Không set maximumSize -> generator có thể tự chọn kích thước mặc định
    /// - Không set appliesPreferredTrackTransform -> có thể sai hướng xoay
    /// - Không set apertureMode -> dùng chế độ mặc định (production aperture, có thể bị crop)
    func extractRawFrame(videoUrl: URL, at time: TimeInterval) async throws -> UIImage {
        let asset = AVURLAsset(url: videoUrl)
        let generator = AVAssetImageGenerator(asset: asset)
        // Cố tình KHÔNG cấu hình gì — giữ nguyên hành vi mặc định của hệ thống.
        let cmTime = CMTime(seconds: time, preferredTimescale: 600)
        let cgImage = try await copyCGImage(generator: generator, at: cmTime)
        // scale mặc định (không ép về 1.0) để phản ánh đúng những gì generator trả về "y nguyên".
